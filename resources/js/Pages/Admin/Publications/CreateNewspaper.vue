...
function submit() {
    form.post(route('admin.publications.newspapers.store'), {
        forceFormData: true,
    });
}
</script>
...
<template>
    <Head title="New Newspaper Issue" />
    <AdminLayout>
        <template #header>
            <div class="flex items-center gap-3">
                ...
                <h2 class="text-xl font-semibold text-slate-900 dark:text-white">New Newspaper Issue</h2>
            </div>
        </template>
        ...
        <form @submit.prevent="submit" class="space-y-6 max-w-4xl">
            <div class="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-6 space-y-5">
                <div>
                    ...
                    <input v-model="form.title" type="text" placeholder="Newspaper title..."
                ...
                <div>
                    ...
                    <input v-model="form.issue_number" type="text" placeholder="e.g. Vol. 3, Issue 5"
                        class="w-full border border-slate-300 dark:border-slate-600 rounded-lg px-4 py-2.5 text-sm bg-white dark:bg-slate-700 text-slate-900 dark:text-white placeholder-slate-400 focus:ring-2 focus:ring-indigo-500 focus:border-transparent outline-none transition" />
                </div>
                <div>
                ...
                <div>
                    ...
                    <textarea v-model="form.body" rows="15" placeholder="Write newspaper content here..."
            ...
            <div class="flex items-center gap-3">
                ...
                <Link :href="route('admin.publications.index', { tab: 'newspapers' })"
