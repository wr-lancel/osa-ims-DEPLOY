    ...
    <StudentLayout>
        ...
        <div class="space-y-6">
            ...
            <div class="bg-white rounded-lg border border-gray-200 shadow-sm p-6">
                <div class="flex flex-wrap gap-4 mb-4">
                    <div>
                        ...
                        <select
                            id="filter-category"
                            v-model="category"
                            class="block rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 max-w-xs"
                        >
                            <option
                ...
                <div v-if="complaints.data && complaints.data.length > 0" class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-gray-200">
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            <tr v-for="c in complaints.data" :key="c.complaint_id" class="hover:bg-gray-50">
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">#{{ c.complaint_id }}</td>
                                <td class="px-6 py-4 text-sm text-gray-900">{{ c.subject }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ c.category }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ c.date_submitted }}</td>
                ...
                <div v-else class="text-center py-8">
                    <p class="text-sm text-gray-500">You have no complaints yet.</p>
                    <Link
                        :href="route('student.discipline.complaints.create')"
                        class="mt-2 inline-block text-indigo-600 hover:text-indigo-900 text-sm font-medium"
                    >
                        Submit a complaint
                    </Link>
                </div>
