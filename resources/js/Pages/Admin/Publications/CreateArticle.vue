...
function submit() {
    form.post(route('admin.publications.articles.store'), {
        forceFormData: true,
    });
}
</script>
    ...
    <AdminLayout>
        <template #header>
            <div class="flex items-center gap-3">
                <Link :href="route('admin.publications.index')" class="text-slate-400 hover:text-slate-600 dark:hover:text-slate-300 transition">
            ...
            <div class="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-6 space-y-5">
                ...
                <div>
                    ...
                    <textarea v-model="form.excerpt" rows="2" placeholder="Short summary (optional)..."
                        class="w-full border border-slate-300 dark:border-slate-600 rounded-lg px-4 py-2.5 text-sm bg-white dark:bg-slate-700 text-slate-900 dark:text-white placeholder-slate-400 resize-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent outline-none transition" />
                </div>
                ...
                <div>
                    ...
                    <textarea v-model="form.body" rows="15" placeholder="Write article content here..."
