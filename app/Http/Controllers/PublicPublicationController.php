...
class PublicPublicationController extends Controller
{
    ...
    private function formatArticle(PublicationArticle $article): array
    {
        return [
            ...
            'body' => $article->body,
            'cover_image' => $article->cover_image ? Storage::url($article->cover_image) : null,
    ...
    private function formatNewspaper(PublicationNewspaper $newspaper): array
    {
        return [
            ...
            'body' => $newspaper->body,
            'cover_image' => $newspaper->cover_image ? Storage::url($newspaper->cover_image) : null,
    ...
    private function formatGallery(PublicationGallery $gallery): array
    {
        return [
            ...
            'cover_image' => $gallery->cover_image ? Storage::url($gallery->cover_image) : null,
            ...
            'photos' => $gallery->relationLoaded('photos') ? $gallery->photos->map(fn($p) => [
                'photo_id' => $p->photo_id,
                'image_path' => Storage::url($p->image_path),
                'caption' => $p->caption,
            ]) : [],
