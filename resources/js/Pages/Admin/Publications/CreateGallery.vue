...
function submit() {
    form.post(route('admin.publications.galleries.store'), { forceFormData: true });
}
</script>
    ...
    <AdminLayout>
        <template #header>
            <div class="flex items-center gap-3">
                ...
                <h2 class="text-xl font-semibold text-slate-900 dark:text-white">New Photo Gallery</h2>
            </div>
        </template>
        ...
        <form @submit.prevent="submit" class="space-y-6 max-w-4xl">
            <div class="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-6 space-y-5">
                <div>
                    ...
                    <input v-model="form.title" type="text" placeholder="Gallery title..."
                ...
                <div>
                    ...
                    <textarea v-model="form.description" rows="3" placeholder="Describe this gallery..."
                ...
                <div>
                    ...
                    <img v-if="coverPreview" :src="coverPreview" class="h-32 w-full object-cover rounded-lg mb-2 border border-slate-200 dark:border-slate-700" />
                ...
                <div>
                    <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5">Upload Photos</label>
                    <input type="file" accept="image/*" multiple @change="onPhotosChange"
                        class="block w-full text-sm text-slate-500 file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-medium file:bg-indigo-50 file:text-indigo-700 hover:file:bg-indigo-100 dark:file:bg-indigo-900/30 dark:file:text-indigo-400 transition" />

                    <div v-if="photoPreviews.length" class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3 mt-4">
            ...
            <div class="flex items-center gap-3">
                <button type="submit" :disabled="form.processing"
                    ...
                    {{ form.processing ? 'Saving...' : 'Save Gallery' }}
                </button>
                <Link :href="route('admin.publications.index', { tab: 'galleries' })"
