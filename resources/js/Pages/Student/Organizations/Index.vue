    ...
    <StudentLayout>
        ...
        <div class="space-y-6">
            ...
            <div v-if="officerOrganizations && officerOrganizations.length > 0">
                <h3 class="text-lg font-semibold text-gray-900 mb-4">My Organizations (Officer)</h3>
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    <Link
                        v-for="org in officerOrganizations"
                        :key="org.org_id"
                        :href="route('student.organizations.show', org.org_id)"
                        class="group bg-indigo-50 border border-indigo-200 rounded-lg p-5 hover:shadow-md transition-shadow"
                    >
                        <div class="flex items-start justify-between">
                            <div>
                                <h4 class="font-semibold text-indigo-900 group-hover:text-indigo-700">
                                    {{ org.org_name }}
                                </h4>
                                <p class="text-sm text-indigo-600 mt-1">{{ org.org_code }}</p>
                            </div>
                            <span class="inline-flex px-2 py-1 text-xs font-semibold rounded-full bg-indigo-200 text-indigo-800">
                                {{ org.officer_role }}
                            </span>
                        </div>
                        <p v-if="org.type" class="text-sm text-indigo-700 mt-3">{{ org.type }}</p>
            ...
            <!-- All Organizations -->
            <div>
                <h3 class="text-lg font-semibold text-gray-900 mb-4">All Organizations</h3>
                ...
                <div v-if="filteredOrganizations.length > 0" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    <Link
                        ...
                        :key="org.org_id"
                        :href="route('student.organizations.show', org.org_id)"
                        class="group bg-white border border-gray-200 rounded-lg p-5 hover:shadow-md transition-shadow"
                    >
                        <div class="flex items-start justify-between">
                            <div>
                                ...
                                <p class="text-sm text-gray-500 mt-1">{{ org.org_code }}</p>
                            </div>
                            <span
                                v-if="org.is_officer"
                        ...
                        <p v-if="org.description" class="text-sm text-gray-600 mt-3 line-clamp-2">
                            {{ org.description }}
                        </p>
                ...
                <div v-else class="bg-white rounded-lg border border-gray-200 shadow-sm p-8 text-center">
